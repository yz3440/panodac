"""ID Module for ERP - Implicit Depth Representations with isolated positional encoding."""
...
from .layers import AttentionLayerIsoPE, PositionEmbeddingSineERP, _get_activation_cls, get_norm


class ISDHead(nn.Module):
    """Implicit Surface Decoder Head for ERP images."""

    def __init__(
        self, depth: int, pixel_dim: int = 256, query_dim: int = 256,
        pe_dim: int = 64, num_heads: int = 4, output_dim: int = 1,
        expansion: int = 2, activation: str = "silu", norm: str = "LN",
        eps: float = 1e-6, pe_mlp: bool = False,
    ):
        super().__init__()
        self.depth = depth
        self.eps = eps
        ...
        for i in range(self.depth):
            setattr(
                self, f"cross_attn_{i+1}",
                AttentionLayerIsoPE(
                    sink_dim=pixel_dim, hidden_dim=pixel_dim, source_dim=query_dim,
                    output_dim=pixel_dim, num_heads=num_heads, dropout=0.0,
    ...
    def forward(
        self, feature_map: torch.Tensor, idrs: torch.Tensor,
        lat_range: torch.Tensor, long_range: torch.Tensor
    ):
...
class ISD(nn.Module):
    """Implicit Surface Decoder - Multi-resolution for ERP."""

    def __init__(
        self, num_resolutions, depth, pixel_dim=128, query_dim=128,
        num_heads: int = 4, output_dim: int = 1, expansion: int = 2,
        activation: str = "silu", norm: str = "torchLN",
    ):
        super().__init__()
        ...
        for i in range(num_resolutions):
            setattr(
                self, f"head_{i+1}",
                ISDHead(
                    depth=depth, pixel_dim=pixel_dim, query_dim=query_dim,
                    num_heads=num_heads, output_dim=output_dim,
                    expansion=expansion, activation=activation, norm=norm,
                ),
            )
...
class AFP(nn.Module):
    """Adaptive Feature Pooling module for ERP images."""

    def __init__(
        self, num_resolutions: int, depth: int = 3, pixel_dim: int = 256,
        pe_dim: int = 64, latent_dim: int = 256, num_latents: int = 128,
        num_heads: int = 4, activation: str = "silu", norm: str = "torchLN",
        expansion: int = 2, eps: float = 1e-6, pe_mlp: bool = False,
        top_k_ratio: Optional[float] = None, top_ks: Optional[list] = None,
    ):
        super().__init__()
        self.num_resolutions = num_resolutions
        self.iters = depth
        ...
        for i in range(self.num_resolutions):
            setattr(self, f"pixel_pe_{i+1}", PositionEmbeddingSineERP(pe_dim // 2))
            setattr(self, f"mu_{i+1}", nn.Parameter(torch.randn(1, self.num_slots, latent_dim + pe_dim)))
            if self.pe_mlp:
                setattr(
                    self, f"mlp_pe_{i+1}",
                    nn.Sequential(
        ...
        for j in range(self.iters):
            for i in range(self.num_resolutions):
                setattr(
                    self, f"cross_attn_{i+1}_d{1}",
                    AttentionLayerIsoPE(
                        sink_dim=latent_dim, hidden_dim=latent_dim, source_dim=pixel_dim,
                        output_dim=latent_dim, num_heads=num_heads, dropout=0.0,
                        pre_norm=True, sink_competition=False,
                    ),
                )
                setattr(
                    self, f"mlp_cross_{i+1}_d{1}",
                    nn.Sequential(
    ...
    def forward(
        ...
        lat_range: torch.Tensor, long_range: torch.Tensor,
        attn_masks: Optional[Tuple[torch.Tensor, ...]] = None
    ) -> Tuple[torch.Tensor, ...]:
        ...
        for i in range(self.num_resolutions):
            ...
            for _ in range(self.iters):
                ...
                idrs[i] = idrs[i] + getattr(self, f"cross_attn_{i+1}_d{1}")(
                    idrs[i].clone(), feature_maps_flat[i], attn_mask, top_t=top_k
                )
                idrs[i] = torch.cat([
                    idrs[i][..., :self.latent_dim] + getattr(self, f"mlp_cross_{i+1}_d{1}")(
                        idrs[i][..., :self.latent_dim].clone()
                    ),
    ...
    def build(cls, config):
        ...
        top_k_ratio = config["model"]["afp"].get("top_k_ratio", None)
